...
class GAE(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        node_features, edge_index, edge_features, node_positions = train_batch
        z = self.encoder(node_features, edge_index, edge_features)
        train_recon_edge_loss, train_auroc, train_ap = self.decode_and_evaluate(z, torch.ones(edge_index.shape[1]))
        train_recon_coord_loss = self.decoder.recon_coord_loss(z, node_positions)
        ...
        wandb.log({"train_total_loss":train_total_loss,"train_recon_edge_loss":train_recon_edge_loss, "train_auroc":train_auroc, "train_ap":train_ap})
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        wandb.log({"val_total_loss":val_total_loss,"val_recon_edge_loss":val_recon_edge_loss, "val_auroc":val_auroc, "val_ap":val_ap})
